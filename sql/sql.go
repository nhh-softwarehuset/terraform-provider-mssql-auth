...
func (f factory) GetConnector(prefix string, data *schema.ResourceData) (interface{}, error) {
	...
	if admin, ok := data.GetOk(prefix + "access_token.0"); ok {
		admin := admin.(map[string]interface{})
		connector.AccessToken = &AccessToken{
			AccessToken: admin["access_token"].(string),
		}
	}
...
func (c *Connector) connector() (driver.Connector, error) {
	...
	if c.Login != nil || c.AzureLogin != nil || c.AccessToken != nil {
		...
		if c.AccessToken != nil {
			...
			if c.AccessToken.AccessToken == "" {
				...
				if err != nil {
					log.Info().Err(err).Msg("Error from access token")
					return nil, err
				}
				log.Info().Msg("Access token generated")
				c.AccessToken.AccessToken = string(accesstoken)
			}
			return mssql.NewAccessTokenConnector(connectionString, func() (string, error) { return c.AccessToken.AccessToken, nil })
		}
